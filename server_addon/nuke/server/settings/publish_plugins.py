...
from ayon_server.settings import BaseSettingsModel, ensure_unique_names, task_types_enum
...
class NodeModel(BaseSettingsModel):
    ...
    nodeclass: str = Field("", title="Node class")
    dependent: str = Field("", title="Incoming dependency")
...
class ExtractThumbnailModel(BaseSettingsModel):
    ...
    nodes: list[NodeModel] = Field(default_factory=list, title="Nodes (deprecated)")
    reposition_nodes: list[ThumbnailRepositionNodeModel] = Field(
        title="Reposition nodes", default_factory=list
    )
...
class ReformatNodesRepositionNodes(BaseSettingsModel):
    node_class: str = Field(title="Node class")
    knobs: list[KnobModel] = Field(default_factory=list, title="Node knobs")
...
class BakingStreamModel(BaseSettingsModel):
    ...
    add_custom_tags: list[str] = Field(title="Custom tags", default_factory=list)


class ExtractReviewDataBakingStreamsModel(BaseSettingsModel):
    enabled: bool = Field(title="Enabled")
...
class PublishPuginsModel(BaseSettingsModel):
    ...
    ValidateContainers: OptionalPluginModel = Field(
        title="Validate Containers", default_factory=OptionalPluginModel
    )
    ...
    ValidateScript: OptionalPluginModel = Field(
        title="Validate Script", default_factory=OptionalPluginModel
    )
    ...
    ExtractReviewDataLut: ExtractReviewDataLutModel = Field(
        title="Extract Review Data Lut", default_factory=ExtractReviewDataLutModel
    )
    ExtractReviewDataBakingStreams: ExtractReviewDataBakingStreamsModel = Field(
        title="Extract Review Data Mov",
        default_factory=ExtractReviewDataBakingStreamsModel,
    )
...
DEFAULT_PUBLISH_PLUGIN_SETTINGS = {
    ...
    "ValidateCorrectAssetName": {"enabled": True, "optional": True, "active": True},
    "ValidateContainers": {"enabled": True, "optional": True, "active": True},
    "ValidateKnobs": {
        "enabled": False,
        "knobs": "\n".join(
            ["{", '    "render": {', '        "review": true', "    }", "}"]
    ...
    "ValidateOutputResolution": {"enabled": True, "optional": True, "active": True},
    "ValidateGizmo": {"enabled": True, "optional": True, "active": True},
    "ValidateBackdrop": {"enabled": True, "optional": True, "active": True},
    "ValidateScript": {"enabled": True, "optional": True, "active": True},
    "ExtractThumbnail": {
        "enabled": True,
        "use_rendered": True,
        ...
        "nodes": [
            {
                ...
                "dependency": "",
                "knobs": [
                    {"type": "text", "name": "type", "text": "to format"},
                    {"type": "text", "name": "format", "text": "HD_1080"},
                    {"type": "text", "name": "filter", "text": "Lanczos6"},
                    {"type": "boolean", "name": "black_outside", "boolean": True},
                    {"type": "boolean", "name": "pbb", "boolean": False},
                ],
            }
        ],
        "reposition_nodes": [
            {
                ...
                "knobs": [
                    ...
                    {"type": "text", "name": "format", "text": "HD_1080"},
                    {"type": "text", "name": "filter", "text": "Lanczos6"},
                    {"type": "bool", "name": "black_outside", "boolean": True},
                    {"type": "bool", "name": "pbb", "boolean": False},
                ],
            }
        ],
    },
    "ExtractReviewData": {"enabled": False},
    "ExtractReviewDataLut": {"enabled": False},
    "ExtractReviewDataBakingStreams": {
        ...
        "outputs": [
            {
                "name": "baking",
                "filter": {"task_types": [], "product_types": [], "product_names": []},
                "read_raw": False,
    ...
    "ExtractSlateFrame": {
        ...
        "key_value_mapping": {
            ...
            "f_submitting_for": {"enabled": True, "template": "{intent[value]}"},
            "f_vfx_scope_of_work": {"enabled": False, "template": ""},
        },
    },
    "IncrementScriptVersion": {"enabled": True, "optional": True, "active": True},
}
