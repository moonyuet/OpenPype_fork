...
class ExtractReviewDataBakingStreams(publish.Extractor):
    ...
    def process(self, instance):
        ...
        staging_dir = os.path.normpath(os.path.dirname(instance.data["path"]))

        instance.data["stagingDir"] = staging_dir

        self.log.info("StagingDir `{0}`...".format(instance.data["stagingDir"]))
        ...
        with maintained_selection():
            ...
            for o_name, o_data in self.outputs.items():
                ...
                self.log.debug(
                    "f_families `{}` > families: {}".format(f_families, families)
                )

                self.log.debug(
                    "f_task_types `{}` > task_type: {}".format(f_task_types, task_type)
                )

                self.log.debug("f_subsets `{}` > subset: {}".format(f_subsets, subset))
                ...
                if f_subsets and not any(re.search(s, subset) for s in f_subsets):
                    continue

                self.log.info(
                    "Baking output `{}` with settings: {}".format(o_name, o_data)
                )
                ...
                exporter = plugin.ExporterReviewMov(
                    self, instance, o_name, o_data["extension"], multiple_presets
                )
                ...
                self.log.debug("__ generated_repres: {}".format(generated_repres))
        ...
        if generated_repres:
            ...
            instance.data["families"].remove("review")
            self.log.info(
                ("Removing `review` from families. " "Not available baking profile.")
            )
            self.log.debug(instance.data["families"])

        self.log.debug("_ representations: {}".format(instance.data["representations"]))
